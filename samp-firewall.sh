...
#   sudo bash samp-firewall.sh status   # muestra estado actual
#   sudo bash samp-firewall.sh save    
...
geoblock_latam() {
    ...
    # si tu comunidad es de latam, puedes` bloquear todo lo que no sea latam y algunos paises europeos
...
show_status() {
    ...
    grep "$LOG_PREFIX" /var/log/kern.log 2>/dev/null | tail -20 || \
        echo "No hay logs aún (ver rsyslog)"
    echo ""
...
case "$1" in
    ...
    save)
        ...
        ;;
    *)
        echo "Uso: $0 {start|stop|status|save}"
        echo ""
